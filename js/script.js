...
const headerResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    ...
    const navRows = document.querySelectorAll(".header-nav__row");

    Constants.ROOT.style.setProperty("--header-wrapper-block-size", headerWrapperBlockSize);

    navItems?.forEach(navItem => {
      const { bottom } = navItem.getBoundingClientRect();

      navItem.style.setProperty("--nav-item-bottom", `${bottom}px`);
    });

    navRows?.forEach(navRow => {
      const { height } = navRow.getBoundingClientRect();
      const navItem = navRow.closest(".header-nav__item");

      navItem.style.setProperty("--nav-row-height", `${height}px`);
    });
  });
});
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/products-slider.js
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/brands-slider.js


const brandsSlider = new core(".brands-slider", {
  modules: [Keyboard, Pagination,],
  pagination: {
    el: ".brands-slider .slider-pagination",
    clickable: true,
    dynamicBullets: true,
  },
  keyboard: {
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/reviews-slider.js
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/categories-mouse-event.js


/** @type {HTMLDivElement} */
const headerCategories = document.querySelectorAll(".header-categories");
const max768 = matchMedia("(max-width: 768px)")

headerCategories.forEach(headerCategory => {
  /** @type {HTMLLIElement} */
  const navItem = headerCategory.closest(".header-nav__item");

  navItem?.addEventListener("mouseenter", () => {
    if (!max768.matches) scrolling.lock();
  });

  navItem?.addEventListener("mouseleave", () => {
    if (!max768.matches) scrolling.unlock();
  });
});

max768.addEventListener("change", event => {
  const { matches } = event;

  if (matches) scrolling.unlock();
});

;// CONCATENATED MODULE: ./src/js/scripts/scripts/categories-click-event.js
/** @type {HTMLDivElement} */
const headerNavRows = document.querySelectorAll(".header-nav__row");
const categories_click_event_max768 = matchMedia("(max-width: 768px)");

headerNavRows?.forEach(headerNavRow => {
  headerNavRow.addEventListener("click", function () {
    if (!categories_click_event_max768.matches) {
      ...
      if (currentRow.classList.contains("active")) {
        ...
        headerNavRows.forEach(headerNavRow => {
          headerNavRow.classList.toggle("active", headerNavRow === currentRow);
        });
      }
    }
  });
...
document.addEventListener("click", event => {
  if (!categories_click_event_max768.matches) {
    ...
    if (!target.closest(".header-nav__item")) {
      const activeHeaderNavRow = document.querySelector(".header-nav__row.active");

      activeHeaderNavRow?.classList.remove("active");
    }
  }
});
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/show-search-input.js
/** @type {HTMLButtonElement} */
const searchButton = document.querySelector(".header-icons__button");
...
document.addEventListener("click", event => {
  ...
  if (!target.closest(".header-icons__button") && !target.closest(".header-form--fixed")) {
    const searchForm = document.querySelector(".header-form--fixed.show");
...
const burgerFormResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    const { borderBoxSize, target } = entry;
    const { blockSize } = borderBoxSize[0];
    const headerNav = target.closest(".header-nav");

    headerNav.style.setProperty("--burger-form-height", `${blockSize}px`);
  });
});

const burgerForm = document.querySelector(".header-form--burger");

if (burgerForm) burgerFormResizeObserver.observe(burgerForm);
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/product-buttons-height.js
const productButtonsResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    const { target, borderBoxSize } = entry;
    const { blockSize } = borderBoxSize[0];
    const productCard = target.closest(".product");

    productCard?.style.setProperty("--product-buttons-height", `${blockSize}px`);
  });
});
...
productButtons?.forEach(productButton => {
  productButtonsResizeObserver.observe(productButton);
});

;// CONCATENATED MODULE: ./src/js/scripts/scripts/brands-filter-actions.js
const brandsFilterResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    const { target } = entry;
    const brandsFilterList = target.querySelector(".brands-filter__list");
    const firstBrandLink = brandsFilterList?.querySelector(".brands-filter__link");

    if (firstBrandLink) {
      const brandsFilterListBlockSize = brandsFilterList.scrollHeight;
      const brandsFilterListRowGap = parseInt(getComputedStyle(brandsFilterList).rowGap);
      const firstBrandLinkBlockSize = parseInt(getComputedStyle(firstBrandLink).blockSize);
      const brandsFilterListBlockSizeSum = brandsFilterListRowGap + firstBrandLinkBlockSize * 2 + 5;

      target.classList.toggle("brands-filter__inner--rolled", brandsFilterListBlockSizeSum < brandsFilterListBlockSize);
      brandsFilterList.style.setProperty("--brands-filter-list-height", `${brandsFilterListBlockSize}px`);
      brandsFilterList.style.setProperty("--brands-filter-list-max-height", `${brandsFilterListBlockSizeSum}px`);
    }
  });
});

const brandsFilterInner = document.querySelector(".brands-filter__inner");

if (brandsFilterInner) {
  brandsFilterResizeObserver.observe(brandsFilterInner);

  /** @type {HTMLButtonElement} */
  const brandsFilterButton = brandsFilterInner.querySelector(".brands-filter__button");

  brandsFilterButton?.addEventListener("click", function () {
    brandsFilterInner.classList.toggle("brands-filter__inner--expanded");
  });
}
